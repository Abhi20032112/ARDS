...
import { faHome, faInfoCircle, faTools, faStar, faPhone } from '@fortawesome/free-solid-svg-icons';
...
const Navbar = () => {
  ...
  const navItems = [
    ...
    { name: 'Our Work', path: '/our-work', icon: faTools },
          ...
          <div className="hidden md:flex items-center ml-auto gap-[30px]">
            {navItems.map((item) => (
                <Link
                  key={item.name}
                  to={item.path}
                  className={`relative text-sm font-semibold nav-underline transition-colors duration-300 rounded-md whitespace-nowrap ${isActive(item.path) ? 'active text-indigo-600' : 'text-gray-600 hover:text-indigo-600'}`}
                >
                  <span className="inline-flex items-center space-x-2">
                    {/* hide decorative icon on desktop to keep spacing consistent */}
                    {item.icon && (
                      <FontAwesomeIcon icon={item.icon} className="h-3 w-3 text-gray-500 md:hidden" />
                    )}
              ...
              {navItems.map((item) => (
                ...
                >
                  <span className="inline-flex items-center space-x-3">
                    {item.icon && <FontAwesomeIcon icon={item.icon} className="h-4 w-4 text-gray-500" />}
                    <span>{item.name}</span>
                  </span>
                </Link>
              ))}
