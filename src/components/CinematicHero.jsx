...
export default function CinematicHero(){
 ...
 const words=['AI, ERP & Cloud','Systems For','Serious Business','Growth'];
 return <section className="cinematic-hero" ref={root} onMouseMove={move}>
  ...
  <div className="site-shell cinema-layout">
   <motion.div className="cinema-copy" style={{x:shiftX,y:shiftY}}>
    <motion.div className="cinema-eyebrow" initial={{opacity:0,y:10}} animate={{opacity:1,y:0}}><span/><I.Sparkles/> ENTERPRISE SOFTWARE | BUILT IN INDIA</motion.div>
    ...
    <motion.p initial={{opacity:0,y:18}} animate={{opacity:1,y:0}} transition={{delay:.65}}>Alpenrose Digital Solutions builds secure AI automation, ERP platforms, websites, apps and cloud systems for schools, hospitals, manufacturers and growing businesses.</motion.p>
    ...
    <motion.div className="cinema-stats" initial={{opacity:0}} animate={{opacity:1}} transition={{delay:.95}}>{[['50+','Projects'],['20+','Solutions'],['ERP + AI','Core focus'],['Secure','By design'],['Human','Support']].map(([v,l])=><div key={l}><b>{v}</b><span>{l}</span></div>)}</motion.div>
   </motion.div>
